...
class TempWeekList extends React.Component {
  ...
  componentDidUpdate() {}
  render() {
    const tempList = !this.props.temps7Days
      ? null
      : this.props.temps7Days.days_letters.map((day, index) => {
          // index = Number(index);
          ...
          return (
            <li key={index} className={status}>
              <img
                className="day-icon"
                src={this.props.temps7Days.icons[index]}
                alt={this.props.temps7Days.weathers[index]}
              />
              <span className="day-name">{day}</span>
              <span className="day-temp">
                {Math.round(this.props.temps7Days.tempsNight[index])}°C
              </span>
            </li>
          );
        });
