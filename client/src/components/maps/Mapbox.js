...
mapboxgl.accessToken =
  "pk.eyJ1Ijoiam5yZG1ubiIsImEiOiJja2wxN3VtY28zaDdlMm5xbjV5Znh0YnBpIn0.s0Cz8vhJe3T1N2wvocwFzw";
...
class Mapbox extends React.Component {
  ...
  mapLookup(event) {
    ...
    this.map.flyTo({
      center: [Number(long.value), Number(lat.value)],
      zoom: 9,
    });
  }
  ...
  isFavorite(cityId) {
    let index;
    let favorite = this.props.favoriteCities
      ? this.props.favoriteCities.filter((favoriteCity, i) => {
    ...
    return (
      <div key={uuidv4()} className={`card-city row ml-3 mr-1 mt-3`}>
        <div className="city-header w-100 d-flex justify-content-between align-items-center px-3 py-1">
          <Link className="city-header-name" to={`/details/${city._id}`}>
            <h4>{city.name}</h4>
          </Link>
        ...
        <div className="d-flex justify-content-end align-items-center w-100">
          ...
          <Link to={`/details/${city._id}`} className="button-removed">
            City Weather
          </Link>
        </div>
      </div>
        ...
        {!this.state.contentContainerWidth ? null : (
          ...
          >
            {this.props.searched && this.state.searched
              ? this.mapDragTo(this.props.cities[0].loc.coordinates)
              : ""}
            {this.props.searched ? this.removeAllPreviousMarkers() : ""}
            {!this.props.cities || !this.map
              ? ""
            ...
            {!this.props.favoriteCities
              ? ""
              : this.props.favoriteCities.forEach((city) => {
                  this.generateMarker(
                    city.loc.coordinates[0],
                    city.loc.coordinates[1],
                    this.map,
