...
export const submitVisitedForm = ({
  fromDate,
  toDate,
  travellersNum,
  cost,
...
export const addFavoriteCity = (cityId) => async (dispatch) => {
  ...
  if (res.data.success)
    dispatch({ type: ADDED_FAVORITE_CITY, payload: res.data });
};
...
export const deleteFavoriteCity = (cityId, index) => async (dispatch) => {
  ...
  if (res.data.success)
    dispatch({ type: DELETED_FAVORITE_CITY, payload: { cityId, index } });
};
